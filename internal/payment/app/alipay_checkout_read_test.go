...
func TestGetCheckoutReadsAlipayOriginalOrderThroughTrustedH5Session(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			got, err := service.GetCheckout(context.Background(), domain.ProviderAlipay, payment.MerchantOrderNo, "authorized-payment-session")
			...
			for _, token := range []string{"wrong-payment-session-01", "mini-payment-session-01"} {
				if _, err = service.GetCheckout(context.Background(), domain.ProviderAlipay, payment.MerchantOrderNo, token); !errors.Is(err, paymentport.ErrConflict) {
			...
			if _, err = service.GetCheckout(context.Background(), domain.ProviderAlipay, payment.MerchantOrderNo, "authorized-payment-session"); !errors.Is(err, paymentport.ErrUnavailable) {
...
func TestGetCheckoutScopesMerchantReferenceToProvider(t *testing.T) {
	store := &storeStub{checkoutPayments: map[domain.Provider]domain.Payment{
		domain.ProviderWeChatPay: {ID: 1, Provider: domain.ProviderWeChatPay, Channel: domain.ChannelH5Official, MerchantOrderNo: "M-collision", PayerIdentityID: 4, PayerCustomerID: 11, BeneficiaryCustomerID: 11, Status: domain.StatusPaid},
		domain.ProviderAlipay:    {ID: 2, Provider: domain.ProviderAlipay, Channel: domain.ChannelAlipayWap, MerchantOrderNo: "M-collision", PayerIdentityID: 4, PayerCustomerID: 11, BeneficiaryCustomerID: 11, Status: domain.StatusPaid},
	}}
	sessions := checkoutReadSessionStub{actors: map[string]paymentport.SessionActor{
	...
	for provider, wantID := range map[domain.Provider]int64{domain.ProviderWeChatPay: 1, domain.ProviderAlipay: 2} {
		got, err := service.GetCheckout(context.Background(), provider, "M-collision", "authorized-payment-session")
		if err != nil || got.PaymentID != wantID {
			t.Fatalf("provider %s read wrong payment: %+v %v", provider, got, err)
		}
	}
}
...
func TestGetCheckoutReturnsAlipayHandoffAfterOriginalEffectCompletes(t *testing.T) {
	...
	service := NewService(uowStub{}, store, orderStub{}, sessions, &effectStub{})
	got, err := service.GetCheckout(context.Background(), domain.ProviderAlipay, "M-alipay-48", "authorized-payment-session")
