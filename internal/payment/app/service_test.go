...
func (s *storeStub) GetPaymentByMerchantProvider(_ context.Context, provider domain.Provider, _ string, _ bool) (domain.Payment, error) {
	if s.payment.Provider != "" && provider != s.payment.Provider {
		return domain.Payment{}, paymentport.ErrNotFound
	}
	return s.payment, nil
}
