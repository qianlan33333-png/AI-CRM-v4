...
<body><main class="card" data-v3-public-commerce data-public-commerce-route="detail" data-product-kind="{{.Product.ProductKind}}"><h1 class="visually-hidden">{{.Product.Name}}</h1>{{if gt .Product.ServicePeriodDurationDays 0}}<span class="visually-hidden">服务周期 {{.Product.ServicePeriodDurationDays}} 天</span>{{end}}<section id="detailContent">{{range $index, $image := .Product.Images}}<img class="detail-image" src="{{$image}}" alt="商品详情" decoding="async" {{if eq $index 0}}loading="eager" fetchpriority="high"{{else}}loading="lazy"{{end}}>{{end}}<footer class="checkout-footer"><div class="footer-price"><span class="label">价格</span><strong>¥<span id="detailPrice">{{yuan .Product.PriceMinor}}</span></strong></div><a class="buy" href="{{.Product.PaymentPath}}">{{.Product.BuyButtonText}}</a></footer></section></main></body></html>`))
