...
for(const selector of ['.product','#paymentDetails','#mobilePanel','#shippingPanel','#paymentMethod','#alipayGuide','.checkout-footer']){const panel=checkoutContent.querySelector(selector);if(panel)panel.hidden=true}
...
function resetCompletionView(){alipayGuide.hidden=true;statusBox.className='status';statusBox.replaceChildren();for(const selector of ['.product','#paymentDetails','#mobilePanel','#shippingPanel','#paymentMethod','.checkout-footer']){const panel=checkoutContent.querySelector(selector);if(panel)panel.hidden=false}}
