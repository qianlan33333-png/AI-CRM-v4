...
func (h *PublicHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case r.URL.Path == "/pay/alipay/continue":
		serveAlipayBrowserHandoff(w, r)
...
<section id="paymentMethod" class="panel"><div class="label">支付方式</div>{{if .WeChatPayEnabled}}<label class="method"><span class="wechat-icon" aria-hidden="true">✓</span><span>微信支付</span><input type="radio" name="paymentMethod" value="wechat_pay" checked></label>{{end}}{{if .AlipayEnabled}}<label class="method"><span class="wechat-icon" style="background:#1677ff" aria-hidden="true">支</span><span>支付宝</span><input type="radio" name="paymentMethod" value="alipay" {{if not .WeChatPayEnabled}}checked{{end}}></label>{{end}}</section><section id="alipayGuide" class="alipay-guide" hidden><p id="alipayGuideMessage">原订单已创建，请继续支付宝付款。</p><a id="alipayOpen" class="buy" href="#" rel="noreferrer noopener">继续支付宝支付</a><div class="alipay-actions"><button id="alipayCopy" type="button">复制原付款链接（备用）</button><button id="alipayPaid" type="button">我已支付</button></div><input id="alipayPaymentURL" aria-label="原订单支付宝付款链接" readonly hidden></section><div id="status" class="status" role="status" aria-live="polite"></div>{{if eq .Product.ProductKind "service_period"}}<button id="renew" class="buy restart" hidden>续费</button>{{end}}<footer class="checkout-footer"><div class="footer-price"><span class="label">实付</span><strong id="footerAmount"></strong></div><div id="paymentErrors" class="payment-errors" role="alert" hidden></div><button id="buy" class="buy">立即支付</button></footer>{{end}}</div>
</main><script>document.getElementById('price').textContent=({{.Product.PriceMinor}}/100).toFixed(2);{{if .Payment}}
const button=document.getElementById('buy'),statusBox=document.getElementById('status'),couponField=document.getElementById('coupon'),identityGate=document.getElementById('identityGate'),identityTitle=document.getElementById('identityTitle'),identityMessage=document.getElementById('identityMessage'),authContinue=document.getElementById('authContinue'),checkoutContent=document.getElementById('checkoutContent'),inWechat=/MicroMessenger/i.test(navigator.userAgent),checkoutStorageKey='aicrm.checkout.tab.v2:'+{{.Product.ID}}+':{{.Product.ProductKind}}',promotionContext='{{.Product.PromotionContext}}';authContinue.href='/api/h5/wechat-pay/oauth/start?return_url='+encodeURIComponent(location.pathname+(promotionContext?'?promotion_context='+promotionContext:''))
let purchaseState='available',detailImageObserver;const authFeedback=document.getElementById('authFeedback');function authorizeMissingSession(message){showIdentityGate(message,true)}function showIdentityGate(message,needsAuth=false){const detail=document.getElementById('detailContent');if(detail)detail.hidden=true;checkoutContent.hidden=true;identityGate.hidden=false;identityGate.parentElement.dataset.authState=needsAuth?'required':'checking';authFeedback.hidden=true;if(needsAuth){identityTitle.textContent='登录才能完成支付';identityMessage.textContent='请先完成微信登录验证，验证成功后继续支付。不会自动扣款。';if(message&&/失效|未完成|失败/.test(message)){authFeedback.textContent=message;authFeedback.hidden=false}authContinue.hidden=!inWechat;authContinue.textContent='授权并继续';authContinue.href='/api/h5/wechat-pay/oauth/start?return_url='+encodeURIComponent(location.pathname+(promotionContext?'?promotion_context='+promotionContext:''));authContinue.removeAttribute('aria-disabled')}else if(inWechat){identityTitle.textContent=message==='网络连接超时，请刷新重试'?'页面加载失败':'正在核验微信身份';identityMessage.textContent=message;authContinue.hidden=false;authContinue.textContent='重新加载页面';authContinue.href=location.href;authContinue.removeAttribute('aria-disabled')}else{identityTitle.textContent='请在微信中打开';identityMessage.textContent='请复制当前链接到微信中打开，登录后才能完成支付。';authContinue.hidden=true}}function revealCheckout(){identityGate.hidden=true;identityGate.parentElement.dataset.authState='complete';checkoutContent.hidden=false}function requestDetailImage(image){const source=image.dataset.src;if(!source)return;image.dataset.publicMediaState='loading';let settled=false,loadingTimer;const settle=state=>{if(settled)return;settled=true;clearTimeout(loadingTimer);image.dataset.publicMediaState=state};image.addEventListener('load',()=>settle('loaded'),{once:true});image.addEventListener('error',()=>settle('unavailable'),{once:true});image.src=source;delete image.dataset.src;loadingTimer=setTimeout(()=>{if(!settled)image.dataset.publicMediaState='delayed'},10000)}function revealProduct(){const detail=document.getElementById('detailContent');if(!detail)return;checkoutContent.hidden=true;detail.hidden=false;const images=Array.from(detail.querySelectorAll('img[data-src]'));const first=images.shift();if(first)requestDetailImage(first);if(images.length){if(typeof IntersectionObserver==='function'){detailImageObserver?.disconnect();detailImageObserver=new IntersectionObserver(entries=>{for(const entry of entries){if(!entry.isIntersecting)continue;detailImageObserver.unobserve(entry.target);requestDetailImage(entry.target)}},{rootMargin:'900px 0px'});for(const image of images)detailImageObserver.observe(image)}else for(const image of images)requestDetailImage(image)}document.getElementById('detailPrice').textContent=({{.Product.PriceMinor}}/100).toFixed(2)}
...
function checkoutKey(payload,binding){const record=checkoutRecord();if(record.state==='valid')return record.value;if(record.state==='invalid')throw requestFailure('checkout_checkpoint_invalid','原订单恢复信息异常，已保留，请联系管理员核对');if(record.state==='unavailable')return null;const created={key:crypto.randomUUID(),merchant_order_no:'',create_attempted:false,payload:checkoutPayload(payload),session_binding:checkoutBinding(binding)};return created.payload&&created.session_binding&&writeCheckout(created)?created:null}
const alipayGuide=document.getElementById('alipayGuide'),alipayGuideMessage=document.getElementById('alipayGuideMessage'),alipayPaymentURL=document.getElementById('alipayPaymentURL');function selectedPaymentMethod(){return document.querySelector('input[name=paymentMethod]:checked')?.value||''}function restorePaymentMethod(checkpoint){const method=checkpoint?.payload?.provider||'wechat_pay';const option=document.querySelector('input[name=paymentMethod][value='+method+']');if(option)option.checked=true}function showAlipayGuide(handoff,orderNo){let target;try{target=new URL(handoff?.redirectUrl)}catch(_){throw new Error('原订单付款链接暂不可用，请稍后查询')}if(target.protocol!=='https:'||target.host!=='openapi.alipay.com'||target.pathname!=='/gateway.do'||target.username||target.password||target.hash||!['alipay.trade.wap.pay','alipay.trade.page.pay'].includes(target.searchParams.get('method'))||['method','app_id','sign','biz_content'].some(key=>target.searchParams.getAll(key).length!==1||!target.searchParams.get(key)))throw new Error('原订单付款链接不安全，请联系客服核对');const checkpoint=readCheckout();if(!checkpoint||checkpoint.merchant_order_no!==orderNo)throw new Error('原订单标识已变化，请刷新后核对');alipayPaymentURL.value=target.href;const open=document.getElementById('alipayOpen');open.href=inWechat?'/pay/alipay/continue#'+encodeURIComponent(target.href):target.href;alipayGuide.hidden=false;if(typeof alipayGuide.scrollIntoView==='function')alipayGuide.scrollIntoView({block:'center',behavior:'smooth'});statusBox.textContent='原订单已创建，请继续完成付款';alipayGuideMessage.textContent=inWechat?'点击下方按钮，按提示在系统浏览器打开原订单；付款后回本页确认结果。':'点击下方按钮打开支付宝，付款后回本页确认结果。';button.disabled=true;button.textContent='已生成订单'}document.getElementById('alipayCopy').addEventListener('click',async()=>{if(!alipayPaymentURL.value)return;try{await navigator.clipboard.writeText(alipayPaymentURL.value);alipayGuideMessage.textContent='已复制。请到系统浏览器地址栏粘贴打开原订单链接。'}catch(_){alipayPaymentURL.hidden=false;alipayPaymentURL.focus();alipayPaymentURL.select();alipayGuideMessage.textContent='自动复制失败，请手动复制下方原付款链接。'}});document.getElementById('alipayPaid').addEventListener('click',async()=>{const checkpoint=readCheckout();if(!checkpoint?.merchant_order_no||checkpoint.payload.provider!=='alipay')return;try{const value=await requestJSON(checkoutStatusURL(checkpoint,checkpoint.merchant_order_no),{credentials:'same-origin'});if(value.status==='paid'){retainPaidCheckout(checkpoint.merchant_order_no);alipayGuide.hidden=true;showCompletionAction(value.completion_action);button.disabled=true;button.textContent='已购买'}else alipayGuideMessage.textContent='服务端尚未确认收款，请完成付款后再查询原订单；不会创建新订单。'}catch(_){alipayGuideMessage.textContent='暂时无法确认原订单付款结果，请稍后重试。'}});const sleep=ms=>new Promise(resolve=>setTimeout(resolve,ms));function requestFailure(code,message,httpStatus=0){const error=new Error(message);error.code=code;error.httpStatus=httpStatus;return error}
function definitiveCreateRejection(error){if(!error)return false;if(error.httpStatus===401&&error.code==='payment_session_required')return true;if(error.httpStatus===403&&error.code==='cross_site_request')return true;const knownCodes=['invalid_request','invalid_product','not_found','payment_provider_mismatch','conflict'];if(error.httpStatus>=400&&error.httpStatus<500&&knownCodes.includes(error.code))return true;return error.httpStatus===503&&['payment_provider_disabled','payment_h5_oauth_disabled'].includes(error.code)}
async function requestJSON(url,options={}){const controller=new AbortController(),timeout=setTimeout(()=>controller.abort(),12000);try{const response=await fetch(url,{...options,signal:controller.signal});let body={};try{body=await response.json()}catch(_){}const fail=(code,message)=>requestFailure(code,message,response.status);if(response.status===401)throw fail('payment_session_required','请先完成微信授权');if(!response.ok){const code=body.code||body.error||'';if(code==='payment_provider_disabled'||code==='payment_h5_oauth_disabled')throw fail(code,'支付服务暂未启用');if(code==='session_mismatch')throw fail(code,'付款授权已变化，原订单标识已保留；请恢复原授权后继续');if(code==='already_purchased')throw fail(code,'您已购买本课程');if(code==='purchase_pending')throw fail(code,'支付状态需要核验，请稍后重试');if(code==='cross_site_request')throw fail(code,'页面来源校验未通过');if(code==='conflict')throw fail(code,'商品或手机号状态不符合购买要求');if(code==='invalid_request'||code==='invalid_product'||code==='not_found'||code==='payment_provider_mismatch')throw fail(code,'订单未创建，请检查商品、授权或填写信息后重试');throw fail(code,'请求失败')}return body}catch(error){if(error&&error.name==='AbortError')throw requestFailure('request_timeout','网络连接超时，请刷新重试');throw error}finally{clearTimeout(timeout)}}
...
button.addEventListener('click',async()=>{if(purchaseState==='owned')return;button.dataset.feedback='true';setTimeout(()=>{button.dataset.feedback='false'},220);button.disabled=true;let checkpoint=null,previousCreateAttempt=false,createPostStarted=false;try{checkpoint=readCheckout();const retrying=!!checkpoint;if(!checkpoint){const method=selectedPaymentMethod();if(!method)throw new Error('当前没有可用的支付方式');const level='{{.Product.ContactCollectionLevel}}'||'none',fields=validateCheckoutFields(level);const payload={product_id:{{.Product.ID}},product_kind:'{{.Product.ProductKind}}',beneficiary_selection:'payer_self',coupon_claim_id:Number(couponField&&couponField.value)||0,contact_collection_level:level};if(method==='alipay'){payload.provider='alipay';payload.channel=inWechat?'alipay_wap':'alipay_page'}if(promotionContext)payload.promotion_context=promotionContext;if(fields.mobile)payload.mobile='+86'+fields.mobile;if(fields.shipping)Object.assign(payload,fields.shipping);checkpoint=checkoutKey(payload,await currentCheckoutBinding(true));if(!checkpoint)throw new Error('无法保存本次订单恢复信息，请检查浏览器存储后重试')}previousCreateAttempt=checkpoint.create_attempted===true;restorePaymentMethod(checkpoint);updateAmounts();if(checkpoint.legacy_unbound){if(!checkpoint.merchant_order_no)throw requestFailure('legacy_checkpoint_unbound','旧版订单恢复标识缺少付款会话绑定，已保留原标识，请勿重新下单');statusBox.textContent='正在恢复原订单…';await poll(checkpoint.merchant_order_no);return}if(checkpoint.merchant_order_no){statusBox.textContent='正在恢复原订单…';await poll(checkpoint.merchant_order_no);return}const currentBinding=await currentCheckoutBinding();if(currentBinding!==checkpoint.session_binding)throw requestFailure('session_mismatch','付款授权已变化，原订单标识已保留；请恢复原授权后继续');statusBox.textContent=retrying?'正在安全重试原请求…':'正在创建订单…';checkpoint.create_attempted=true;if(!writeCheckout(checkpoint))throw requestFailure('checkout_checkpoint_unavailable','无法保存原订单恢复信息，请勿重新下单');createPostStarted=true;const created=await requestJSON(checkoutApiRoot(checkpoint),{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json','Idempotency-Key':checkpoint.key},body:JSON.stringify({...checkpoint.payload,checkout_session_binding:checkpoint.session_binding})});if(typeof created.merchant_order_no!=='string'||!created.merchant_order_no)throw requestFailure('order_creation_unknown','订单创建结果尚未核实');checkpoint.merchant_order_no=created.merchant_order_no;if(!writeCheckout(checkpoint))throw new Error('无法保存原订单恢复信息，请勿重新下单');statusBox.textContent='正在读取付款信息…';await poll(created.merchant_order_no)}catch(error){if(error&&error.code==='already_purchased'){showOwnedPurchase();return}if(error&&error.code==='payment_session_required'){if(checkpoint&&!checkpoint.merchant_order_no&&!previousCreateAttempt&&error.httpStatus===401){clearCheckout();originalAmount=null;updateAmounts()}authorizeMissingSession('本次微信授权已失效，请重新授权后继续。');return}if(checkpoint&&!checkpoint.merchant_order_no&&createPostStarted&&!previousCreateAttempt&&definitiveCreateRejection(error)){clearCheckout();originalAmount=null;updateAmounts();button.textContent='立即支付';statusBox.textContent=error&&error.code==='cross_site_request'?'订单未创建，页面来源校验未通过。请从配置的 H5 地址重新打开页面后重试。':'订单未创建，服务端已拒绝本次请求。请检查信息后重新提交。'}else if(checkpoint&&!checkpoint.merchant_order_no&&!(error&&['payment_session_required','session_mismatch','legacy_checkpoint_unbound'].includes(error.code))){statusBox.textContent='订单创建结果尚未核实，原请求已保留。点击重试会复用同一请求，不会重复下单。';button.textContent='重试原请求'}else statusBox.textContent=error instanceof Error?error.message:'支付结果暂不可用，请使用原订单重试';button.disabled=false}});void bootstrapCheckout();{{end}}</script></body></html>`))
