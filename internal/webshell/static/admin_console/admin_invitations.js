...
if (root) {
  ...
  let plans = [], selected = [], editing = null, offset = 0, total = 0, optionOffset = 0, busy = false;
  ...
  async function loadPlans() {
    ...
    plans = v.items;
    $("createInvitation").disabled = !v.write_enabled;
    ...
    $("invitationRows").innerHTML = plans.map((p) => {
      const g = cache.get(p.current_chat_id);
      return `<tr><td>${esc(p.name)}</td><td>${!p.token ? "\u65E7\u9080\u8BF7" : p.mode === "single" ? "\u56FA\u5B9A\u5355\u7FA4" : `\u591A\u7FA4 \xB7 ${p.threshold} \u4EBA`}</td><td>${p.current_chat_id ? `${esc(groupName(g))}<br><code>${esc(p.current_chat_id)}</code>` : "\u2014"}</td><td>${g?.observed_at ? g.member_count : "\u2014"}<br>${esc(when(g?.observed_at))}</td><td>${esc(names[p.state] || p.state)}</td><td><button class="aud-btn" data-edit="${p.id}">${p.token ? "\u7F16\u8F91" : "\u7ED1\u5B9A\u5347\u7EA7"}</button><button class="aud-btn" data-link="${p.id}">\u590D\u5236\u94FE\u63A5</button><button class="aud-btn" data-qr="${p.id}" ${!p.token ? "disabled" : ""}>\u4E0B\u8F7D\u5165\u53E3\u7801</button><button class="aud-btn" data-history="${p.id}">\u5207\u6362\u8BB0\u5F55</button></td></tr>`;
  ...
  root.addEventListener("click", (e) => {
    ...
    run(async () => {
      ...
      if (b.dataset.qr) {
        const p = plans.find((p2) => p2.id === Number(b.dataset.qr)), qr = (0, import_qrcode_generator.default)(0, "M");
        qr.addData(p.join_url);
        qr.make();
        const a = document.createElement("a");
        a.href = qr.createDataURL(8, 32);
        a.download = `\u9080\u8BF7\u5165\u53E3-${p.id}.gif`;
        a.click();
      }
