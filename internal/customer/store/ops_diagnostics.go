...
import (
	...
	ownerport "github.com/qianlan33333-png/AI-CRM-v3/internal/customer/port"
	"time"
)

// OpsDiagnosticReader reads only customer-owned tables and never changes them.
...
func (r *OpsDiagnosticReader) ReadOpsDiagnosticCounts(ctx context.Context, at time.Time) (map[string]int64, error) {
	if r == nil || r.pool == nil || at.IsZero() {
		return nil, errors.New("customer diagnostics unavailable")
	}
	...
	for _, q := range []struct{ key, sql string }{{"projection_conflicts", `SELECT count(*) FROM customer_directory_projection WHERE activation_status='conflict' AND $1::timestamptz IS NOT NULL`}, {"projection_stale", `SELECT count(*) FROM customer_directory_projection WHERE activation_status='stale' AND $1::timestamptz IS NOT NULL`}} {
		var n int64
