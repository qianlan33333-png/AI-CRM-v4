...
3. 一个候选交付可独立上线、可回退的完整行为或明确缺陷，相关回归测试与代码一起提交。按行为拆分，不按行数拆分；涉及 UI 时编码前使用 Product Design。
...
预备机使用合成数据测试，生产只在数据库迁移前备份。生产技术安装读回、真实支付和扫码等业务验收分别记录。发布失败由单独的 `gpt-6-luna` max agent 诊断和修复。
