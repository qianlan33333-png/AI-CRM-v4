...
- 能力成本适配要求同一 CI run/attempt 的 GitHub jobs JSON（`gh run view RUN_ID --attempt ATTEMPT --json databaseId,attempt,jobs`），绑定 plan job 开始至稳定 required `check` 完成的全 CI wall time；backend 单 lane 耗时只能支持类别性能样本，不能满足能力 CI 成本。它同时复用候选 backend 收据、国内发布器既有 `state.json`、构建 `domestic-release.json` 与 `last_release_timings_seconds.total`；发布状态需就绪，`processed_sha`、`deployed_source_sha` 与 `prod_installed_sha` 必须一致，安装 manifest 与 source tree 精确匹配。比较该能力的全部候选 PR CI 加部署总时长之和与一份已绑定基线，不取 PR 均值。缺少完整 CI wall time、source/manifest/phase timing/能力基线时保持原门禁。
