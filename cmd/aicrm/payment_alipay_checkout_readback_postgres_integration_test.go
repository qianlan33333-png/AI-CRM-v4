...
func assertLegacyAlipayIntentOrderSnapshotRecovery(t *testing.T, fixture *productExternalPushChromiumFixture, created alipayCheckoutCreateResult, channel paymentdomain.Channel) {
	...
	if err := fixture.application.pool.Native().QueryRow(fixture.ctx, `SELECT source_ref_digest FROM payment_provider_intents WHERE payment_id=$1`, created.PaymentID).Scan(&source); err != nil {
		t.Fatal("read synthetic Alipay intent source")
	}
	...
	orderRepository, err := orderstore.NewPostgreSQL(fixture.application.pool.Native(), uow)
	if err != nil {
		t.Fatal("create Order repository")
	}
	...
	intent, err := read()
	if err != nil || intent.Subject != "" || intent.OrderID != created.OrderID || intent.ProductID != "" {
	...
	if err := uow.Within(fixture.ctx, func(tx context.Context) error {
		...
		t.Fatal("read immutable Order checkout snapshot")
	}
	material, err := (paymentprovider.DBMaterialLoader{UOW: uow, Intents: repository, Checkouts: orders}).Load(fixture.ctx, kind, source)
