...
donor_dir="${AICRM_TX01_DONOR_DIR:-/tmp/aicrm-v2-readonly.OSvgXU}"
donor_sha="6bfbe5816bb89913c70adaca87d6a486260e016e"
hash_file="$repo_root/docs/migration/transaction/pr-tx01-donor-sha256.txt"

fail() { printf 'TX01 donor manifest check: FAIL: %s\n' "$*" >&2; exit 1; }
...
test -d "$donor_dir/.git" || fail "missing donor checkout: set AICRM_TX01_DONOR_DIR"
...
while IFS=' ' read -r expected reference; do
  ...
  case "$source_path" in
    web/src/admin/templates/orders.html|web/src/admin/templates/orderDetail.html)
...
[[ "$source_count" -eq 11 ]] || fail "expected 11 donor files, found $source_count"
...
if rg -n 'github\.com/qianlan33333-png/AI-CRM-v2|replace[[:space:]]+github\.com/qianlan33333-png/AI-CRM-v2' \
  "$repo_root/go.mod" "$repo_root/go.sum" "$repo_root/internal" "$repo_root/cmd" --glob '*.go' --glob 'go.mod' --glob 'go.sum' >/dev/null; then
  fail "forbidden v2 runtime import detected"
fi

printf 'TX01 donor manifest check: PASS (donor %s; %d sources; %d byte-exact active templates; no v2 runtime import)\n' "$resolved_sha" "$source_count" "$active_count"
