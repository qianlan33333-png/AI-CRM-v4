...
import { DonorViewError, recoverPartialMaterialization } from './donor-source-views.mjs';

const repository = path.dirname(path.dirname(fileURLToPath(import.meta.url)));
const usage = 'usage: node scripts/recover-donor-source-views.mjs [--root DIRECTORY] [--index REPOSITORY_RELATIVE_PATH]';
...
try {
  ...
  console.log(JSON.stringify(recoverPartialMaterialization(path.resolve(args.root), args.index), null, 2));
} catch (error) {
