...
make_release() {
  ...
  for binary in aicrm aicrm-operation-cycle-runner aicrm-operation-cycle-result wecom-archive-sdk-runner migrate-platform migrate-access-role-convergence check-enterprise-directory migrate-river migrate-phone-identities migrate-identity-phone-vault migrate-survey-v2 migrate-commerce-history migrate-message-archive migrate-order-attribution migrate-order-distribution-qualification migrate-referral-sales migrate-automation-operations migrate-v2-config-definitions migrate-v2-runtime-config-releases migrate-v2-commerce-external-push-history migrate-open-platform migrate-media-legacy-materials migrate-hxc-daily-lessons migrate-channel-history migrate-v2-customer-tag-history migrate-radar-v2 migrate-sidebar-history migrate-owner-handoff-history bootstrap-automation-operations; do
